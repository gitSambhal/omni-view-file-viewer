...
export const ReaderSwitcher: React.FC<ReaderSwitcherProps> = ({ activeTab, onSelectReader }) => {
  ...
  return (
    <div className="flex items-center gap-1.5 relative" ref={dropdownRef}>
      ...
      {isTextual && (
        <div className="hidden sm:flex items-center bg-slate-200/60 dark:bg-slate-800/60 p-0.5 rounded-lg text-[11px] border border-slate-200/80 dark:border-slate-700/80">
          {isHtmlCapable && (
            <button
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-white dark:bg-[#0f172a] hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700/80 transition-all shadow-2xs cursor-pointer"
        title="Change how this file is rendered"
      >
        <SlidersHorizontal className="w-3 h-3 text-slate-400" />
        <span className="text-slate-400 dark:text-slate-500 text-[11px]">View:</span>
        <ActiveIcon className={`w-3.5 h-3.5 ${activeOption.color}`} />
        <span className="font-semibold max-w-[110px] truncate">{activeOption.label.split('/')[0].trim()}</span>
        <ChevronDown className={`w-3 h-3 text-slate-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>

      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 top-full mt-1.5 w-72 max-h-96 overflow-auto bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-50 p-1 space-y-0.5 animate-in fade-in duration-100">
          <div className="px-2.5 py-1.5 text-[11px] font-semibold text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <span>Reader Mode</span>
            <span className="font-mono text-[10px] uppercase bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">{activeTab.extension || 'file'}</span>
          </div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-start gap-2.5 p-2 rounded-lg text-left transition-all cursor-pointer ${
                    isSelected
                      ? 'bg-blue-50 dark:bg-blue-950/40 text-blue-900 dark:text-blue-200 border border-blue-200/80 dark:border-blue-800/50'
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className={`text-xs ${isSelected ? 'font-semibold text-blue-600 dark:text-blue-400' : 'font-medium'}`}>
                        {opt.label}
                      </span>
                      {isSelected && (
                        <span className="text-[10px] bg-blue-500/10 text-blue-600 dark:text-blue-400 px-1.5 py-0.2 rounded font-semibold">Active</span>
                      )}
                    </div>
